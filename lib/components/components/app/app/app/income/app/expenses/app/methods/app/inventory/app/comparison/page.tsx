// ---------------------------------------------------------------------------------
// FILE: app/comparison/page.tsx  (A vs B)
// ---------------------------------------------------------------------------------
'use client';
...
export default function Page(){
  return <AuthGuard><Comparison/></AuthGuard>;
}
...
function Comparison(){
  ...
  const toCat = (xs:any[]) => {
    ...
    for (const r of xs) m.set(r.category, (m.get(r.category)||0) + Number(r.amount||0));
    return Array.from(m, ([k,v]) => ({ category:k, total:v }));
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="space-y-2">
        <div className="text-sm font-semibold">Range A</div>
        <Filters f={fA} setF={setFA} categories={Array.from(new Set(rows.map(r=>r.category).filter(Boolean)))} methods={Array.from(new Set(rows.map(r=>r.method).filter(Boolean)))} />
      </section>

      <section className="space-y-2">
        <div className="text-sm font-semibold">Range B</div>
        <Filters f={fB} setF={setFB} categories={Array.from(new Set(rows.map(r=>r.category).filter(Boolean)))} methods={Array.from(new Set(rows.map(r=>r.method).filter(Boolean)))} />
      </section>

      <div className="grid md:grid-cols-3 gap-4">
        <KPI title="Income A" value={totalIncomeA} />
        <KPI title="Expenses A" value={totalExpenseA} />
        <KPI title="Net A" value={netA} />
      </div>
      <div className="grid md:grid-cols-3 gap-4">
        <KPI title="Income B" value={totalIncomeB} />
        <KPI title="Expenses B" value={totalExpenseB} />
        <KPI title="Net B" value={netB} />
      </div>
      ...
      <section className="space-y-4">
        ...
        <div className="h-72 bg-white rounded-2xl shadow p-3">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={catIncome}>
              <XAxis dataKey="name" />
              <YAxis />
              <Tooltip />
              <Legend />
              <Bar dataKey="A" name="Range A" />
              <Bar dataKey="B" name="Range B" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </section>
...
function merge(a: { category: string; total: number }[], b: { category: string; total: number }[]){
  const m = new Map<string, { name: string; A: number; B: number }>();
  for (const r of a) m.set(r.category, { name: r.category, A: r.total, B: 0 });
  for (const r of b) {
    const found = m.get(r.category);
    if (found) found.B = r.total; else m.set(r.category, { name: r.category, A: 0, B: r.total });
  }
